...
| Linux toolchain | clang, ninja, GTK 3 dev headers, `libsqlite3-dev` | Phase 3. |
...
### Desktop only (Phase 1–3)

| Package | Purpose |
|---|---|
| `window_manager` | Frameless window, min size, transparency |
| `macos_window_utils` | macOS-specific toolbar styles, vibrancy (Phase 1) |
| `tray_manager` | System tray icon (Phase 3, Linux) |
| `hotkey_manager` | Global show/hide hotkey (Phase 3+, optional) |
| `multi_split_view` | Resizable sidebar/inspector (optional, Phase 2 polish) |
...
| `flutter_secure_storage` | No secrets to store in v1. Reintroduce with sync (Phase 5). |
