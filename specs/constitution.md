...
3. **Local-first, sync-resilient.** SQLite (via drift) is the source of truth on every device. The app must be 100% functional offline. All primary keys are assigned locally (no server-generated IDs), so a full re-push of every local row to a clean remote is always possible — if a remote loses data, the local DB is the complete recovery source. Cloud sync via Supabase (Phase 10) and Realtime row-level LWW merge (Phase 11) layer on top without violating this invariant.
...
- **Decided:** SQLite (drift) is the source of truth. Supabase was kept out of Phases 1–7 and reintroduced as opt-in cloud sync in Phase 10; Phase 11 added Realtime + row-level LWW merge with soft-delete tombstones.
...
- **Revisit if:** Supabase pricing or availability constraints push us off the platform — the local-first invariant guarantees the codebase can swap backends without data loss.
...
- **Decided:** `health` tracking type is Android-only and reads from Health Connect via the `health` package (Phase 12). The pill is hidden on macOS/Linux. Real macOS Health bridge remains out of scope.
